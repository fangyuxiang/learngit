...
class Login:
    ...
    def __init__(self, session):
        ...
        upFilePath = "\\".join(filePath.split("\\")[0:-2])
        config_path = os.path.join(upFilePath, 'config.ini')
    ...
    def do_login(self):
        ...
        if re.match(r"^1\d{10}$", self.username):
            ...
            # 2017/10/29：如果原有的code登录不成功，可以尝试将：captcha_type:on 用于替换remeber_me
            postdata = {
                '_xsrf': self.get_xsrf(),
                'password': self.password,
                'remember_me': 'true',
        ...
        try:
            ...
            print("login_post_data:", postdata)
            print("login_result:", login_page.text)
            login_json = json.loads(login_page.text.encode('latin-1').decode('unicode-escape'))
            print("login_result_decode:", login_json)
            # 需要输入验证码 r = 0为登陆成功代码
